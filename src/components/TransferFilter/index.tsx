...
const TransferFilter: FC<TransferFilterProps> = ({
  local,
  operationDisabled,
  pagination,
  sourceActions,
  ...
  const sourceLoadData = async (params: TransferFilterLoadDataParams) => {
    if (params.exclude) {
      delete (params as Partial<TransferFilterLoadDataParams>).exclude;
    }
  ...
  const moveToLeft = () => {
    ...
    if (
      ...
      internalValue.include =
        internalValue.include?.filter(item => !set.has(item)) ?? null;
