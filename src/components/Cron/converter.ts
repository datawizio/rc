 ...
 */
export const cronToString = (parts: string[]) => {
  return parts.join(" ");
};
...
export const getPeriodFromCronparts = (cronParts: number[][]): PeriodType => {
  if (cronParts[3].length > 0) return "year";
  if (cronParts[2].length > 0) return "month";
  if (cronParts[4].length > 0) return "week";
  if (cronParts[1].length > 0) return "day";
  if (cronParts[0].length > 0) return "day";
  return "day";
};
 ...
 */
export const parsePartString = (str: string, unit: Unit) => {
  if (str === "*" || str === "*/1") {
    return [];
  }
 ...
 */
export const replaceAlternatives = (
  str: string,
  min: number,
  alt?: string[]
) => {
 ...
 */
export const fixSunday = (values: number[], unit: Unit) => {
 ...
 */
export const parseRange = (rangeStr: string, context: string, unit: Unit) => {
...
/**
 * Finds an element from values that is outside the range of unit.
 */
export const outOfRange = (values: number[], unit: Unit) => {
 ...
 */
export const parseStep = (step: string, unit: Unit) => {
 ...
 */
export const applyInterval = (values: number[], step?: number) => {
  if (step) {
 ...
 */
export const parsePartArray = (arr: number[], unit: Unit) => {
 ...
 */
export const getStep = (values: number[]) => {
  if (values.length > 2) {
 ...
 */
export const isInterval = (values: number[], step: number) => {
 ...
 */
export const isFullInterval = (values: number[], unit: Unit, step: number) => {
 ...
 */
export const toRanges = (values: number[]) => {
